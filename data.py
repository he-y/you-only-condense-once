...
class TensorDataset(torch.utils.data.Dataset):
    def __init__(self, data, labels, transform=None):
        # data: NxCxHxW tensor
        self.data = data.detach().cpu().float()
...
class ImageFolder(datasets.DatasetFolder):
    ...
    def find_subclasses(self, nclass=100, phase=0, seed=0):
        ...
        if seed == 0:   # default is seed=0, take subclasses in order
...
class MultiIndexEpochsDataLoader(torch.utils.data.DataLoader):
    """Multi epochs data loader
    """
    def __init__(self, is_test_set=False, *args, **kwargs):
        ...
        if not self.is_test_set:
            type_ = self.dataset.dataset.data[0].dtype
        else:
            type_ = self.dataset[0][0].dtype

        if type_ == torch.uint8:
            self.convert = transforms.ConvertImageDtype(torch.float)

        # if self.dataset.dataset.data[0].device == torch.device('cpu'):
        #     self.device = 'cpu'
        # else:
        #     self.device = 'cuda'
        self.device = 'cuda'
    ...
    def __iter__(self):
        if not self.is_test_set:
            for i in range(len(self)):
                index, (data, target) = next(self.iterator)  # Note the change here.
                if self.convert != None:
                    data = self.convert(data)
                yield index, (data, target)  # Note the change here.
        else:
            for i in range(len(self)):
...
class ClassDataLoader(MultiEpochsDataLoader):
    ...
    def class_sample(self, c, ipc=-1, start_idx=0):
        if ipc > 0:
            indices = self.cls_idx[c][start_idx:start_idx+ipc]
        else:
...
def get_save_img(save_dir, img, unnormalize=True, max_num=200, size=64, nrow=10, dataname='imagenet'):
    img = img[:max_num].detach()
    if unnormalize:
        img = img_denormlaize(img, dataname=dataname)
    img = torch.clamp(img, min=0., max=1.)
    ...
    grid = make_grid(img.cpu(), nrow=nrow)
