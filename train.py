...
from data import load_data, MEANS, STDS, MultiEpochsDataLoader
...
def train(args, model, train_loader, val_loader, plotter=None, logger=None, aim_run=None, model_name=None):
...
def validate(args, val_loader, model, criterion, epoch, logger=None):
    ...
    pred_list = []  # predicted label list
    end = time.time()
    for i, (input, target) in enumerate(val_loader):
        ...
        # gt_list += target.data.cpu().numpy().tolist()
        # pred = output.data.cpu().numpy().tolist()
        # pred_list += np.argmax(pred, axis=1).tolist()
    ...
    # print("Classification report\n", classification_report(gt_list, pred_list))
